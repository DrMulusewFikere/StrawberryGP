...
load("../Analysis_post220605/1. Stucture/strawberryStructure.RData")
mapData <- read.csv("strawberry_mapFile.csv", h=T, stringsAsFactors = F)
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    pop2_NAs_in_genome_tmp <- rawGeno_wzNA_pop2_long_map %>%
       ...
       missingDist_pop1 <- ggplot(pop1_NAs_in_genome_tmp, aes(x=countfactor, y=ID, fill=as.factor(Call))) +
        ...
        ggsave(missingDist_pop1, filename="../Analysis_post220605/2. Imputation/missingDist_population1.png", height=6.5, width=12.8, units="in", dpi=1000)
      ...
      #<<<<<<<<<<<<<<<<<<<<<
        ...
        missingDist_pop2 <- ggplot(pop2_NAs_in_genome_tmp, aes(x=countfactor, y=ID, fill=as.factor(Call))) +
          ...
          ggsave(missingDist_pop2, filename="../Analysis_post220605/2. Imputation/missingDist_population2.png", height=469, width=892, units="mm", dpi=1000)
...
save.image("../Analysis_post220605/2. Imputation/strawberryImputation.RData")
